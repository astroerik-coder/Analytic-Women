<section class="antialiased bg-gray-100 text-black  px-4 mb-5">
    ...
    <div class="mt-8">
        <div class="flex justify-between">
            <div class="text-2xl">Redes Sociales</div>
        </div>
        ...
        <div class="mt-6">
            <div class="flex justify-between">
                ...
                <div>
                    <button type="submit" wire:click="$emitTo('redes-child', 'showCreateForm');"
                        ...
                        <span class="relative text-black group-hover:text-white">Agregar Redes</span>
                    </button>
                </div>
            </div>
            <table class="w-full border-collapse bg-white text-left text-sm text-gray-500 mt-8"
                ...
                <thead class="bg-gray-5">
                    <tr class="px-6 py-4 font-medium text-white bg-primary">
                        ...
                        <td scope="col" class="px-6 py-4">Facebook</td>
                        <td scope="col" class="px-6 py-4">Instagram</td>
                        <td scope="col" class="px-6 py-4">Linkedin</td>
                        <td scope="col" class="px-6 py-4">Tiktok</td>
                ...
                <tbody class="divide-y divide-gray-100 border-t border-gray-100 ">
                    ...
                    <tr class="hover:bg-violet-50">
                        ...
                        <td class="px-6 py-4">{{ $result->FACEBOOK }}</td>
                        <td class="px-6 py-4">{{ $result->INSTAGRAM }}</td>
                        <td class="px-6 py-4">{{ $result->LINKEDIN }}</td>
                        <td class="px-6 py-4">{{ $result->TIKTOK }}</td>
                        <td class="px-6 py-4">
                            <div class="flex justify-mid gap-4">
                                {{-- Editar --}}
                                <button type="submit"
                                    wire:click="$emitTo('redes-child', 'showEditForm', {{ $result->ID_RDS}});"
                                ...
                                {{-- Eliminar --}}
                                <button type="submit"
                                    wire:click="$emitTo('redes-child', 'showDeleteForm', {{ $result->ID_RDS}});"
                                    class="material-icons-round text-base">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
