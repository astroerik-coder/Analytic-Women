    ...
    <div class="mt-8">
        ...
        <div class="mt-6">
            <div class="flex justify-between">
                ...
                <div>
                    <button type="submit" wire:click="$emitTo('empleos-child', 'showCreateForm');"
                        ...
                        <span class="relative text-black group-hover:text-white">Agregar Empleo</span>
                    </button>
                </div>
            </div>
            <table class="w-full border-collapse bg-white text-left text-sm text-gray-500 mt-8"
                ...
                <thead class="bg-gray-5">
                    <tr class="px-6 py-4 font-medium text-white bg-primary">
                        <td scope="col" class="px-6 py-4">Id Emp</td>
                        <td scope="col" class="px-6 py-4">Nombre </td>
                        <td scope="col" class="px-6 py-4">Descripcion</td>
                        <td scope="col" class="px-6 py-4">Modalidad</td>
                        <td scope="col" class="px-6 py-4">Horario</td>
                ...
                <tbody class="divide-y divide-gray-100 border-t border-gray-100 ">
                    ...
                    <tr class="hover:bg-violet-50">
                        ...
                        <td class="px-6 py-4">{{ $result->NOMBRE_EMP }}</td>
                        <td class="px-6 py-4">{{ $result->DESCRIPCION_EMP }}</td>
                        <td class="px-6 py-4">{{ $result->MODALIDAD_EMP }}</td>
                        <td class="px-6 py-4">{{ $result->HORARIO_EMP }}</td>
                        <td class="px-6 py-4">{{ $result->FECHA_PUBLICACION_EMP }}</td>
                        <td class="px-6 py-4">{{ $result->EXPERIENCIA_EMP }}</td>
                        <td class="px-6 py-4">
                            <div class="flex justify-center gap-4">
                                {{-- Editar --}}
                                <button type="submit"
                                    wire:click="$emitTo('empleos-child', 'showEditForm', {{ $result->ID_EMP}});"
                                ...
                                {{-- Eliminar --}}
                                <button type="submit"
                                    wire:click="$emitTo('empleos-child', 'showDeleteForm', {{ $result->ID_EMP}});"
                                    class="material-icons-round text-base">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
