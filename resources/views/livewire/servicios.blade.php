    ...
    <div class="mt-8">
        ...
        <div class="mt-6">
            <div class="flex justify-between">
                ...
                <div>
                    <button type="submit" wire:click="$emitTo('servicios-child', 'showCreateForm');"
                        ...
                        <span class="relative text-black group-hover:text-white">Agregar Servicios</span>
                    </button>
                </div>
            </div>
            ...
            <thead class="bg-gray-5">
                <tr class="px-6 py-4 font-medium text-white bg-primary">
                    <td scope="col" class="px-6 py-4" >
                        <div class="flex items-center">
                            <button wire:click="sortBy('ID_SER')">Id</button>
                ...
                <tbody class="divide-y divide-gray-100 border-t border-gray-100 ">
                    ...
                    <tr class="hover:bg-violet-50">
                        <td class="px-6 py-4" >{{ $result->ID_SER }}</td>
                        <td class="px-6 py-4" >{{ $result->ID_CLT }}</td>
                        <td class="px-6 py-4" >{{ $result->NOMBRE_SER }}</td>
                        <td class="px-6 py-4" >{{ $result->DESCRIPCION_SER }}</td>
                        <td class="px-6 py-4" >
                            <div class="flex justify-end gap-4">
                                {{-- Editar --}}
                                <button type="submit"
                                    wire:click="$emitTo('servicios-child', 'showEditForm', {{ $result->ID_SER}});"
                                ...
                                {{-- Eliminar --}}
                                <button type="submit"
                                    wire:click="$emitTo('servicios-child', 'showDeleteForm', {{ $result->ID_SER}});"
                                    class="material-icons-round text-base">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
