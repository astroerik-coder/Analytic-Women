    ...
    <div class="mt-8">
        ...
        <div class="mt-6">
            <div class="flex justify-between">
                ...
                <div>
                    <button type="submit" wire:click="$emitTo('contactos-child', 'showCreateForm');"
                        ...
                        <span class="relative text-black group-hover:text-white">Agregar Contacto</span>
                    </button>
                </div>
            </div>
            <table class="w-full border-collapse bg-white text-left text-sm text-gray-500 mt-8"
                ...
                <thead class="bg-gray-5">
                    <tr class="px-6 py-4 font-medium text-white bg-primary">
                        ...
                        <td scope="col" class="px-6 py-4">Email1 </td>
                        <td scope="col" class="px-6 py-4">Email2 </td>
                ...
                <tbody class="divide-y divide-gray-100 border-t border-gray-100 ">
                    ...
                    <tr class="hover:bg-violet-50">
                        ...
                        <td class="px-6 py-4">{{ $result->NOMBRE_CONT }}</td>
                        <td class="px-6 py-4">{{ $result->TELEFONO1_CONT }}</td>
                        <td class="px-6 py-4">{{ $result->TELEFONO2_CONT }}</td>
                        <td class="px-6 py-4">{{ $result->EMAIL1_CONT }}</td>
                        <td class="px-6 py-4">{{ $result->EMAIL2_CONT }}</td>
                        <td class="px-6 py-4">
                            <div class="flex justify-center gap-4">
                                {{-- Editar --}}
                                <button type="submit"
                                    wire:click="$emitTo('contactos-child', 'showEditForm', {{ $result->ID_CONT}});"
                                ...
                                {{-- Eliminar --}}
                                <button type="submit"
                                    wire:click="$emitTo('contactos-child', 'showDeleteForm', {{ $result->ID_CONT}});"
                                    class="material-icons-round text-base">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
